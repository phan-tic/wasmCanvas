...
private:
  ...
  int BorderMode = 0; // 0 = None, 1 = Outline, 2 = Inset
  float BorderSizePixel = 0.0f;

  bool ClipsDescendants = false;

  // int LayoutOrder = 0; Layout order for UIListLayout and UIGridLayout stuff TO DO
  ...
  int SizeConstraint = 0;         // 0 = RelativeXY, 1 = RelativeXX, 2 = RelativeYY,
  Vector2 AbsoluteSize{100, 100}; // Read only // TO DO: Add min and max size constraints

  bool Visible = true;
  int ZIndex = 0; // Controls the rendering order of GUI objects. Higher numbers are rendered on top of lower numbers.
...
public:
  ...
  void setSizeConstraint(int sizeConstraint);
